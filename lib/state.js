...
function normalizeDailyMap(daily) {
  ...
  for (const [day, bucket] of Object.entries(daily)) {
    normalized[day] = normalizeDailyBucket(bucket);
  }
...
function normalizeState(rawState) {
  ...
  merged.history.daily = normalizeDailyMap(merged.history.daily);

  if (!isPlainObject(merged.chart)) {
    merged.chart = createDefaultState().chart;
  }

  if (!merged.chart.startDateUtc) {
    merged.chart.startDateUtc = "2026-04-14T00:00:00Z";
  }

  merged.chart.daily = normalizeDailyMap(merged.chart.daily);
